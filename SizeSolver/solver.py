...
class Solver:
    ...
    def totalstrat(self, node):
        """Returns the frequency that each branch is taken with range at a given node."""
        return np.sum(node.strategy, axis=1)/np.sum(node.strategy)
    
    def getCost(self, players=[0,1], total=True):
        """ Cost function of the solver. Specifically, the difference from the current strategy and best.
        Returns cost for each player. Can return cost by combo as well by setting total to False.
        """
        regrets = []
        for pos in players:
            handevs = []
    ...
    def prune(self, node):
        """ Prunes node and resulting children from the tree. """
    ...
    def SizeSolve(self, n=20, goal=.01):
        """ Changes appropriate sizes via betsize gradient and solves new tree to goal nash distance
        at every iteration.
        """
        for i in range(n):
            self.sizestep(self.onodes)
            self.Solve(goal=goal)
    ...
    def showgrad(self, node):
        """ Shows size gradient for a given node. Zero means the size is optimal, positive means size wants to increase."""
        grad = []
        for o in node.options:
            if 'r' in o:
                branch = getattr(node, o)
    ...
    def showsizes(self):
        """ Prints information about nodes in the tree after SizeSolve finishes executing"""
    ...
    def randsizes(self, node, bounds=[20, 150]):
        if node and len(node.sizes) > 0:
            node.sizes = np.array(sorted(np.random.uniform(bounds[0], bounds[1], len(node.sizes))))
        self.tree.adjust()

    def Solve(self, n=500, lr=.4, exp=.7, goal=.01):
        """ Finds the nash equilibrium of a static tree.
            N is the maximum number of gradient steps taken \n
            Goal is the nash distance as a fraction of the pot \n
            Lr is conservative around .4, but optimal lr can vary depending on tree \n
            Exp is exponential weighting in the gradient step and is pretty robustly optimal around .7
        """
